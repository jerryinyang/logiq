...
- **[LOW] Open redirect via `//evil.com` bypasses `startsWith('/')` guard** — `web/src/app/(auth)/login/page.tsx:90`: `redirectTo?.startsWith('/')` — a value like `//evil.com` starts with `/` but is interpreted by browsers as a protocol-relative URL, causing an open redirect. Pre-existing issue in login page from earlier stories.